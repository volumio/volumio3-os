...
## Setup for pioneer5-based boards (sigmastar pioneer5 platform)
...
### Device information
DEVICENAME="pioneer5 SigmaStar Board"
...
write_device_bootloader() {
  # TODO: as of 10/05/2026, bootloader has to be installed manually 
  # on boot0/boot1 eMMC hardware partitions and cannot be shipped
  # along the Volumio image
...
device_image_tweaks() {
  log "Pioneer5: adding service for integrated analog audio mixer setup sstar-asoc-preboot"
  cp "${PLTDIR}/${DEVICEBASE}/fixups/sstar-asoc-preboot.service" "${ROOTFSMNT}/lib/systemd/system"
  cp "${PLTDIR}/${DEVICEBASE}/fixups/sstar-asoc-preboot.sh" "${ROOTFSMNT}/usr/local/bin"

  log "Pioneer5: adding service for eth0 MAC address persistence"
  cp "${PLTDIR}/${DEVICEBASE}/fixups/sstar-eth0-mac.service" "${ROOTFSMNT}/lib/systemd/system"
  cp "${PLTDIR}/${DEVICEBASE}/fixups/sstar-eth0-mac.sh" "${ROOTFSMNT}/usr/local/bin"
...
device_chroot_tweaks() {
  log "Pioneer5: enable sstar-asoc-preboot service"
  systemctl enable sstar-asoc-preboot

  log "Pioneer5: enable sstar-eth0-mac service"
  systemctl enable sstar-eth0-mac
}
...
EOF
  ...
  sed -i "s/bootpart=UUID=/bootpart=UUID=${UUID_BOOT}/g" /boot/extlinux/extlinux.conf
  sed -i "s/datapart=UUID=/datapart=UUID=${UUID_DATA}/g" /boot/extlinux/extlinux.conf
